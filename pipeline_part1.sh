...
cd ~/Murinae/ ## sera à remplacer par #dossier travail

mkdir Hydromyini ## sera à remplacer par #nom groupe
cd Hydromyini
...
# need a .txt file with accession numbers (1 number per line)
...
# need a .csv file with col 1 : species, col 2 : id of individus (accession numbers) , with 1 line per individu
# need a .txt file with list of species (1 specie per line)
...
# need the fasta file with the informations and sequences of the mouse reference exons
# need the fasta files with the informations and UTR5' and 3' sequences of the reference exons
...
cat ../list_all_exons_extremities_genes.txt | parallel -j30 ~/scripts/script_get_exon_seq_sans_utr_V3.sh {} ../list_all_exons_extremities_genes_positions.txt ../blast_UTR_5_vs_exons.txt ../blast_UTR_3_vs_exons.txt ../Mouse_Exons_ref_geneID_exonID_transcriptID_CDSlength_exonrank.fasta 2 >> Mouse_exons_ref_extremities_genes_coding_seq.fasta 2>error.txt
...
# make blast #
cat ~/Murinae/Hydromyini/Data/list_species_Hydromyini.txt | parallel -j10 ~/scripts/script_blast_exons_vs_contigs.sh ~/Murinae/Reference_exons/Mouse_exons_ref_coding_seq_lgmin100.fasta Hydromyini {} 99 85 0.5 4 >results_blast_exons_vs_contigs_Hydromyini.txt 2>errors_blast_exons_vs_contigs_Hydromyini.txt
...
mkdir ${sp}
cd ${sp}
cat ~/Murinae/Hydromyini/Blast/blast_exon_vs_contigs/${sp}/exons_list_blast_${sp}.txt | parallel -j50 ~/scripts/script_blast_reciproque_V2.sh {} ~/Murinae/Hydromyini/Blast/blast_exon_vs_contigs/${sp}/blast_exons_vs_contigs_filtre_${sp}.txt ~/Murinae/Hydromyini/Assembly/trinity_OUT_${sp}.Trinity.fasta ${sp} 1 >out_rec_blast_${sp}.txt 2>warnings_rec_blast_${sp}.txt
cd ../
done
...
cat ~/Murinae/Reference_exons/List_mouse_exons_ref_coding_seq_lgmin100.txt | parallel -j30 ~/scripts/script_concatener_especes_exons.sh {} Hydromyini
...
cd Aligned_Sequences
	# align sequences
cat ~/Murinae/Hydromyini/Sequences/Exons_list.txt | parallel -j40 ~/scripts/script_alignement_omm_macse.sh {} Hydromyini > out_alignment_Hydromyini.txt 2> errors_alignment_Hydromyini.txt
	# move all alignments in the same directory
mkdir alignments

cat ~/Murinae/Hydromyini/Sequences/Exons_list.txt | parallel -j50 ~/scripts/script_mv_align_omm.sh {} Hydromyini
...
cat ~/Murinae/Hydromyini/Aligned_Sequences/alignments/list_exons_align.txt | parallel -j50 ~/scripts/script_remove_ref_from_align.sh {} Hydromyini
...
cat ~/Murinae/Hydromyini/Data/cleaned_pooled_reads/list_indiv_Hydromyini.txt | parallel -j10 ~/scripts/script_mapping.sh 5 {} Hydromyini >out_mapping_Hydromyini.txt 2>errors_mapping_Hydromyini.txt
...
cat ~/Murinae/Hydromyini/Data/list_species_Hydromyini.txt | parallel -j 50 ~/scripts/script_contig_list.sh {} Hydromyini

# calcul heterozygosity
cd ../

cat ~/Murinae/Hydromyini/Data/list_species_Hydromyini.txt | parallel -j 50 ~/scripts/script_angsd.sh {} Hydromyini 2>error_het.txt >out_het.txt
...
cat ~/Murinae/Hydromyini/Data/list_species_Hydromyini.txt | parallel -j 40 ~/scripts/script_table_info_exons_V2.sh {} Hydromyini 2>error.txt >out.txt
...
cat ~/Murinae/Hydromyini/Aligned_Sequences/alignments/alignments_wt_ref/alignments_wt_ref_filtered/Exons_filtered_final_list.txt | parallel -j40 ~/scripts/script_remove_seq_with_stop_codons_V2.sh {} Hydromyini >out_command_stop.txt 2>error_command_stop.txt
