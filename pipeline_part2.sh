...
cat ~/Murinae/Hydromyini/Data/list_species_Hydromyini.txt | parallel -j50 ~/scripts/script_concatenate_exons.sh {} Hydromyini
...
~/scripts/script_concatenate_all_sp_exons.sh Hydromyini
...
iqtree2 -s ~/Murinae/Hydromyini/Aligned_concatenated_sequences/All_seq/Hydromyini_all_seq_NT_aligned_final.fasta  -nt 50 -m GTR -bb 1000 2>error_iqtree.txt >out_iqtree.txt
...
if (($(grep -c ">" $align) < 5)) ; then mv $align not_used ; fi 
done
...
cat ~/Murinae/Hydromyini/Aligned_Sequences/FINAL/Final_genes_list.txt | parallel -j50 ~/scripts/script_arbre_gene.sh Hydromyini {} 2> error_gene_tree.txt >out_gene_tree.txt
...
cat ~/Murinae/Hydromyini/Aligned_Sequences/FINAL/Final_exons_list.txt | parallel -j50 ~/scripts/script_filtre_sup_paralogues.sh {} Hydromyini > tab_lg_tree_for_sup_filter_paralogues.csv
...
cat ~/Murinae/Hydromyini/Aligned_Sequences/FINAL/list_exons_min10sp.txt | parallel -j50 ~/scripts/script_mapNH_bpp.sh Hydromyini {} 2> error_sortie_mapNH.txt > out_mapNH.txt

find -type f -name "*.dnd_1" | cut -f1 -d"_" | cut -f2 -d"/" > list_exons_mapping_ok.txt

cd output
mkdir correct_file_names
cat ../list_exons_mapping_ok.txt | parallel -j50 ~/scripts/transform_mapping_file_name.sh {}
...
cat ~/Murinae/Hydromyini/Substitutions_mapping/list_exons_mapping_ok.txt | parallel -j50 Rscript ~/scripts/script_nb_subst.R {} ~/Murinae/Hydromyini/Substitutions_mapping/output/correct_file_names/ {}_stat_nb_subst.csv 2> errors_script_nb_subst.txt > out_script_nb_subst.txt
...
cat ~/Murinae/Hydromyini/Aligned_Sequences/FINAL/list_exons_sup1000bp.txt | parallel -j50 python3 ~/scripts/script_I_Moran.py {} ~/Murinae/Hydromyini/Substitutions_mapping/output/correct_file_names/ I_Moran_{}.csv 2> errors_script_I_Moran.txt > out_script_I_Moran.txt
...
cat ~/Murinae/Hydromyini/Substitutions_mapping/list_exons_mapping_ok.txt | parallel -j50 ~/scripts/script_simul_stat_V2.sh {} Hydromyini 1000 > out_simul.txt 2> error_simul.txt
...
cat ~/Murinae/Hydromyini/Substitutions_mapping/list_exons_mapping_ok.txt | parallel -j10 Rscript ~/scripts/script_nb_subst_NS.R {} ~/Murinae/Hydromyini/Substitutions_mapping/output/correct_file_names/ {}_stat_nb_subst_NS.csv 2> errors_script_nb_subst_NS.txt > out_script_nb_subst_NS.txt
...
cat ~/Murinae/Hydromyini/Substitutions_mapping/list_exons_mapping_ok.txt | parallel -j10 ~/scripts/script_simul_distrib_subst_NS.sh {} Hydromyini 1000 > sortie_simul_NS.txt 2> error_simul_NS.txt
...
cat ~/Murinae/Hydromyini/Substitutions_mapping/list_exons_mapping_ok.txt | parallel -j10 ~/scripts/script_GC_content.sh {} Hydromyini >out_GC_content.txt 2>errors_GC_content.txt
...
cat ~/Murinae/Hydromyini/Substitutions_mapping/list_genes_mapping_ok.txt | parallel -j15 ~/scripts/
script_num_nodes_tree.sh {} Hydromyini 2> error_rel_br.txt > out_rel_br.txt
...
cat ~/Murinae/Hydromyini/Final_data/list_genes.txt | parallel -j30 python3 ~/scripts/analyse_random_S_V3.py {} ~/Murinae/Hydromyini/Final_Data/ ~/Murinae/Hydromyini/Randomisations/Distrib_S_blockbrep/ ~/Murinae/Hydromyini/Sup_filter_paralogs/paralog_exons.txt >out_analyse_randomS.txt 2>error_analyse_randomS.txt
...
cat ~/Murinae/Hydromyini/Final_data/list_genes.txt | parallel -j30 python3 ~/scripts/script_tab_for_sort_ep.py {} ~/Murinae/Hydromyini/Final_data/ ~/Murinae/Hydromyini/Sup_filter_paralogs/paralog_exons.txt 2>error.txt >out.txt

for file in *_tab_for_sort_ep.csv ; do sed 1d $file ; done > tab_for_sort_ep_all_genes_hydromyini.csv
## AJOUTER TITRE COLONNES ###

### make the sorting ####
~/bin/MLsort tab_for_sort_ep_all_genes_hydromyini.csv optfile.txt posterior_optimum_hydromyini.csv
